...
Map staticVar, funVar, createdVar;
...
void pushFunParm(ObjectType variableType, const char* variableName, bool ptr) {
...
Object* findVariable(const char* variableName) {
    ...
    if (!value || !value->symbol)
        value = (Object*)map_get(&staticVar, (void*)variableName);
    if (!value || !value->symbol)
        return NULL;
...
bool forEnd() {
    ...
    // Loop action code
    fprintf(outFile, "%s", cacheCodeStream[conditionIndex + 1]->data);
    // Loop end
    --indent;
    codeOut("EL%d:", offset);
    ++indent;
    ...
    // Loop condition code
    fprintf(outFile, "%s", cacheCodeStream[conditionIndex]->data);
...
int main(int argc, char* argv[]) {
    ...
    Object endl = {OBJECT_TYPE_STR, 0, 0, &(SymbolData){"endl"}};
    map_putpp(&staticVar, "endl", &endl);
    codeOut(".source %s\n", outputFileName);
    codeOutS(".class public Main\n");
    codeOutS(".super java/lang/Object\n");

    funVar = map_create(objectInfo);
    createdVar = map_create(objectInfo);
    ...
    //     if (obj->symbol->write) {
    //         fprintf(outFile, "%s [%s] \"+r\"(%s)", first ? ":" : ",", (char*)entries->key, (char*)entries->key);
    //         first = false;
    //     }
    // });
    ...
    //     fprintf(outFile, "%c \"%s\"", first ? ':' : ',', tmpRegName[i]);
    //     first = false;
    // }
    // fprintf(outFile, first ? ":\n" : "\n");
